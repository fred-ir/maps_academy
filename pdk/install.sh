...
tar xzf sky130_release_0.1.0.tar.gz
if ($status != 0) then
    echo "Error: Failed to extract PDK tar files!"
    exit 1
endif

tar xzf sky130_scl_9T_0.1.2.tar.gz
if ($status != 0) then
    echo "Error: Failed to extract stdcells tar files!"
    exit 1
endif

tar xzf sky130_die_collateral_1.1.tar.gz
if ($status != 0) then
    echo "Error: Failed to extract die collateral files!"
    exit 1
endif
...
cp pvs_files/techRuleSets sky130_release_0.1.0
if ($status != 0) then
    echo "Error: Failed to copy techRuleSets!"
    exit 1
endif

cp pvs_files/virtuoso6.drc_preset_golden sky130_release_0.1.0
if ($status != 0) then
    echo "Error: Failed to copy virtuoso6.drc_preset_golden!"
    exit 1
endif

cp pvs_files/virtuoso6.lvs_preset_golden sky130_release_0.1.0
if ($status != 0) then
